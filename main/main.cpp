...
int main(int argc, char *argv[]) {
    ...
    GLFWwindow *window = glfwCreateWindow(800, 600, "Comp371 - Lab 03", nullptr, nullptr);
    if (window == nullptr) {
    ...
    while(!glfwWindowShouldClose(window)) {
        ...
        mat4 groundWorldMatrix =
                translate(mat4(1.0f), vec3(0.0f, -0.01f, 0.0f)) * scale(mat4(1.0f), vec3(1000.0f, 0.02f, 1000.0f));
        ...
        mat4 pillarWorldMatrix =
                translate(mat4(1.0f), vec3(0.0f, 10.0f, 0.0f)) * scale(mat4(1.0f), vec3(2.0f, 20.0f, 2.0f));
        glUniformMatrix4fv(worldMatrixLocation, 1, GL_FALSE, &pillarWorldMatrix[0][0]);
        glDrawArrays(GL_TRIANGLES, 0, 36);

        for (int i = 0; i < 20; ++i) {
            for (int j = 0; j < 20; ++j) {
                pillarWorldMatrix = translate(mat4(1.0f), vec3(-100.0f + i * 10.0f, 5.0f, -100.0f + j * 10.0f)) *
                                    scale(mat4(1.0f), vec3(1.0f, 10.0f, 1.0f));
                glUniformMatrix4fv(worldMatrixLocation, 1, GL_FALSE, &pillarWorldMatrix[0][0]);
                glDrawArrays(GL_TRIANGLES, 0, 36);
        ...
        if (cameraFirstPerson) {
            mat4 spinningCubeWorldMatrix = mat4{1.0f};
            mat4 spinningCubeViewMatrix = translate(mat4{1.0}, vec3(0, 0, -1)) *
                    rotate(mat4{1.0f}, radians(spinningCubeAngle), vec3(0, 0, 0));
